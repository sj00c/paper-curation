---
title: "1810_AutoOdom_Learning_Auto-regressive_Proprioceptive_Odometry_fo"
authors:
  - "Changsheng Luo"
  - "Yushi Wang"
  - "Wenhan Cai"
...
arxiv: ""
score: 4.0
essence: "AutoOdom은 자동회귀 학습을 기반으로 하는 2단계 훈련 패러다임으로 다리 로봇의 고유감각 주행거리 추정 성능을 크게 향상시킨 시스템이다. 대규모 시뮬레이션 데이터로 비선형 동역학을 학습하고 제한된 실제 데이터로 sim-to-real 갭을 해결한다."
tags:
...
- 🏛 기반 연구: [[papers/1802_An_Empirical_Evaluation_of_Four_Off-the-Shelf_Proprietary_Vi/review]] — VIO 시스템 벤치마크 결과를 바탕으로 다리 로봇의 고유감각 주행거리 추정 성능을 향상시킨다.
- 🔄 다른 접근: [[papers/2078_Legged_Robot_State-Estimation_Through_Combined_Forward_Kinem/review]] — 다리 로봇 상태 추정에서 하나는 자동회귀 고유감각 방식, 다른 하나는 forward kinematics 방식을 사용한다.
- 🔗 후속 연구: [[papers/2023_InEKFormer_A_Hybrid_State_Estimator_for_Humanoid_Robots/review]] — 자동회귀 학습 기반 주행거리 추정을 하이브리드 상태 추정기로 확장하여 더 정확한 휴머노이드 제어를 가능하게 한다.
- 🏛 기반 연구: [[papers/1849_Contact-Aided_Invariant_Extended_Kalman_Filtering_for_Robot/review]] — 로봇 상태 추정을 위한 contact-aided 칼만 필터링이 AutoOdom의 고유감각 기반 추정의 이론적 기반을 제공한다
- 🏛 기반 연구: [[papers/1802_An_Empirical_Evaluation_of_Four_Off-the-Shelf_Proprietary_Vi/review]] — VIO 시스템의 벤치마크 결과가 고유감각 주행거리 추정 성능 향상을 위한 기초 데이터를 제공한다.
- 🔗 후속 연구: [[papers/2023_InEKFormer_A_Hybrid_State_Estimator_for_Humanoid_Robots/review]] — InEKFormer의 하이브리드 상태 추정 방법이 AutoOdom의 자기회귀 고유수용 추측항법을 보완하여 더 정확한 위치 추정을 가능하게 한다.
- 🔗 후속 연구: [[papers/2078_Legged_Robot_State-Estimation_Through_Combined_Forward_Kinem/review]] — Legged Robot State-Estimation의 접촉 정보 활용을 AutoOdom의 자기회귀 고유수용 추측항법과 결합하여 더 견고한 상태 추정이 가능하다.
