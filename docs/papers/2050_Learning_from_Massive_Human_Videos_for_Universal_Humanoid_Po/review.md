---
title: "2050_Learning_from_Massive_Human_Videos_for_Universal_Humanoid_Po"
authors:
  - "Jiageng Mao"
  - "Siheng Zhao"
  - "Siqi Song"
...
arxiv: ""
score: 4.0
essence: "Humanoid-X는 인터넷의 160,000개 이상의 인간 동영상으로부터 20백만 개의 휴머노이드 로봇 동작을 수집한 대규모 데이터셋이며, UH-1 모델을 통해 텍스트 명령을 휴머노이드 로봇의 제어 신호로 변환하는 범용 언어 조건부 제어를 실현한다."
tags:
...
- 🔄 다른 접근: [[papers/2022_In-N-On_Scaling_Egocentric_Manipulation_with_in-the-wild_and/review]] — 대규모 인간 비디오 데이터 활용에서 에고센트릭 조작 대신 범용 포즈 제어를 위한 다른 데이터 수집 및 학습 접근법을 제시한다.
- 🔗 후속 연구: [[papers/1666_Scaling_Large_Motion_Models_with_Million-Level_Human_Motions/review]] — 20백만 개의 휴머노이드 동작 데이터를 백만 단위 인간 모션과 결합하여 더 대규모의 통합 모션 모델을 구축할 수 있다.
- 🏛 기반 연구: [[papers/1996_Humanoid_Locomotion_as_Next_Token_Prediction/review]] — 다음 토큰 예측 방식의 locomotion 학습 원리가 UH-1 모델의 텍스트-제어 변환 메커니즘에 대한 이론적 기반을 제공한다.
- 🏛 기반 연구: [[papers/1745_Unveiling_the_Impact_of_Data_and_Model_Scaling_on_High-Level/review]] — 대규모 데이터와 모델 스케일링이 고수준 휴머노이드 제어에 미치는 영향 분석 기반
- 🔄 다른 접근: [[papers/1761_Zero-Shot_Whole-Body_Humanoid_Control_via_Behavioral_Foundat/review]] — 범용 휴머노이드 제어에서 행동 기반 모델과 언어 조건부 모델의 다른 접근법
- 🏛 기반 연구: [[papers/1666_Scaling_Large_Motion_Models_with_Million-Level_Human_Motions/review]] — 대규모 인간 비디오 데이터로부터 학습하는 기본적인 방법론을 제공한다.
- 🔄 다른 접근: [[papers/1761_Zero-Shot_Whole-Body_Humanoid_Control_via_Behavioral_Foundat/review]] — 대규모 인간 비디오 데이터 활용에서 zero-shot control과 universal policy라는 서로 다른 일반화 접근법을 사용한다.
- 🔗 후속 연구: [[papers/1814_Being-H0_Vision-Language-Action_Pretraining_from_Large-Scale/review]] — 대규모 인간 비디오 학습 방법론에서 Being-H0가 제시한 physical instruction tuning이 universal policy 개발에 영향을 미쳤다.
- 🏛 기반 연구: [[papers/1860_Deep_Imitation_Learning_for_Humanoid_Loco-manipulation_throu/review]] — 대규모 인간 비디오로부터의 범용 휴머노이드 정책 학습이 TRILL의 deep imitation learning 프레임워크에 필요한 인간 동작 데이터와 학습 방법론을 제공한다.
- 🏛 기반 연구: [[papers/1996_Humanoid_Locomotion_as_Next_Token_Prediction/review]] — 대규모 인간 비디오 학습이 next token prediction 방식의 데이터 기반을 제공한다.
- 🔄 다른 접근: [[papers/2022_In-N-On_Scaling_Egocentric_Manipulation_with_in-the-wild_and/review]] — 대규모 인간 비디오 데이터셋을 활용한 휴머노이드 제어에서 서로 다른 데이터 수집 및 학습 접근법을 비교할 수 있다.
- 🔄 다른 접근: [[papers/2093_Masquerade_Learning_from_In-the-wild_Human_Videos_using_Data/review]] — 범용 휴머노이드 정책을 위한 대규모 인간 비디오와 야생 인간 비디오 데이터 편집이라는 다른 접근법을 제시한다.
