---
title: "1961_H-RDT_Human_Manipulation_Enhanced_Bimanual_Robotic_Manipulat"
authors:
  - "Hongzhe Bi"
  - "Lingxuan Wu"
  - "Tianwei Lin"
...
arxiv: ""
score: 4.0
essence: "H-RDT는 대규모 egocentric 인간 조작 데이터로 사전학습하고 모듈식 action encoder/decoder를 통해 다양한 로봇에 fine-tuning하는 두 단계 diffusion transformer 기반 접근법으로, 로봇 조작 학습을 향상시킨다."
tags:
...
- 🏛 기반 연구: [[papers/1957_GraspDreamer_생성형_인간_시연_기반_기능적_파지_모방_학습/review]] — 생성형 인간 시연 기반 학습이 H-RDT의 대규모 egocentric 데이터 활용에 기반을 제공한다.
- 🔄 다른 접근: [[papers/1903_EgoMimic_Scaling_Imitation_Learning_via_Egocentric_Video/review]] — 둘 다 egocentric 데이터를 통한 모방 학습을 다루지만 H-RDT는 bimanual 조작에, EgoMimic은 일반적인 모방에 초점을 맞춘다.
- 🔄 다른 접근: [[papers/2000_Humanoid_Policy__Human_Policy/review]] — H-RDT와 Humanoid Policy는 모두 대규모 인간 데이터로 사전학습하되, diffusion transformer와 HAT라는 서로 다른 아키텍처를 사용합니다.
- 🏛 기반 연구: [[papers/2164_TWIST2_Scalable_Portable_and_Holistic_Humanoid_Data_Collecti/review]] — TWIST2의 holistic humanoid 데이터 수집 시스템이 H-RDT의 대규모 egocentric 조작 데이터 사전학습을 위한 기반 기술을 제공합니다.
- 🔗 후속 연구: [[papers/1904_EgoVLA_Learning_Vision-Language-Action_Models_from_Egocentri/review]] — H-RDT의 인간 조작 강화 양손 로봇 조작이 EgoVLA의 VLA 모델을 구체적 조작 태스크로 확장한 형태입니다.
- 🔄 다른 접근: [[papers/1906_Embodiment-Aware_Generalist_Specialist_Distillation_for_Unif/review]] — H-RDT의 bimanual 로봇 조작 향상 연구가 단일 정책 기반이 아닌 다른 방식으로 다양한 로봇 제어 문제를 해결하는 접근을 제시한다.
- 🔗 후속 연구: [[papers/1957_GraspDreamer_생성형_인간_시연_기반_기능적_파지_모방_학습/review]] — H-RDT의 인간 조작 사전학습이 GraspDreamer의 기능적 파지 학습을 강화할 수 있다.
- 🔄 다른 접근: [[papers/2000_Humanoid_Policy__Human_Policy/review]] — HAT 기반 통합 상태-행동 공간과 H-RDT의 diffusion transformer는 대규모 인간 데이터 활용을 위한 서로 다른 아키텍처 접근법입니다.
- 🔄 다른 접근: [[papers/2068_Learning_to_Get_Up_Across_Morphologies_Zero-Shot_Recovery_wi/review]] — 다양한 휴머노이드 형태에서 범용적인 낙상 복구와 교차 휴머노이드 보행 사전 훈련이라는 다른 접근법을 제시한다.
- 🏛 기반 연구: [[papers/2159_TrajBooster_Boosting_Humanoid_Whole-Body_Manipulation_via_Tr/review]] — 휠드에서 이족 휴머노이드로의 전이학습 기본 원리가 H-RDT의 인간 시연 강화 이중손 조작에도 적용된다.
