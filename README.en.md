...
**What you need**: a Zotero collection with PDFs, a Zotero API key, a Google API key, and either a ready Claude Code OAuth login or an Anthropic Console API key. Setup defaults to `auto`; it does not force an auth mode or topic.
...
Use the checked-out `bin/paper-curation.mjs` as the single internal/team milestone harness. It uses Node.js built-ins only, and skill installation runs before Python/conda package setup. Public GitHub/npm release, public scanner/license readiness, and the canonical upstream embedding merge are deferred; do not claim public readiness.
...
```bash
# 3) Create current-user config. Never silently reuse ignored fork/worktree residue.
node ./bin/paper-curation.mjs setup --fresh-config

# 4) Diagnose dependencies, network, and the selected Anthropic auth path
node ./bin/paper-curation.mjs doctor --network --anthropic-smoke

# 5) Use one alias printed by setup for a scratch-only bounded smoke
...
# 6) First local curate, forced non-deploy
PAPER_CURATION_NO_DEPLOY=1 node ./bin/paper-curation.mjs run -- --topic <configured-topic> --mode curate --source zotero --no-deploy
```

Setup fetches Zotero collections, supports selecting more than one, and creates an alias for each selection. The alias can represent any Zotero topic profile; docs do not assume repository sample topics. Fresh `config.json` is secret-free: Zotero, Google, and Anthropic credentials come from `.env` or process environment. Use `setup --reuse-config` only after confirming the existing config belongs to the current user and Zotero account.

> **Production warning:** smoke, verification, and local curation use both deployment suppressors. A production curate without them may auto-publish when Cloudflare credentials are configured. Deploy only on an explicit publish request:
>
> ```bash
> node ./bin/paper-curation.mjs run -- --topic <configured-topic> --mode deploy
> ```

To force metered Console billing, put `ANTHROPIC_API_KEY` in `.env` or process environment and use `setup --fresh-config --auth api-key`. Use `--auth oauth` only when intentionally forcing OAuth. `auto` detects OAuth/API-key availability, but setup does not store OAuth tokens or API keys in config.

### After an immutable release

Current first-run docs intentionally use the checkout-local Node harness. After an internal immutable tag or SHA is announced, use only the pinned command from that release note. Public GitHub/npm distribution is deferred, and unqualified `npx --yes github:jehyunlee/paper-curation` is not a first-install command.

### Manual conda fallback

```bash
# Run from an existing maintainer, fork, or pull-request checkout.
cd paper-curation
conda create -n py312 -c conda-forge python=3.12 pip -y
conda activate py312
pip install -r requirements.txt

# Keep real keys out of shell history: store them in .env with an editor.
cp .env.example .env
open -e .env
claude auth login
PYTHONUTF8=1 python pipeline/setup.py --anthropic-auth oauth --no-run

# Metered Console API alternative:
# Add ANTHROPIC_API_KEY to .env with an editor, then run:
...
`OK` means you're ready. To preview the execution plan first, keep deploy suppression on:
...
| Search index builds with empty embeddings | `GOOGLE_API_KEY` not set | Add `GOOGLE_API_KEY` to `.env` with an editor, then re-run — search embeddings use Google `gemini-embedding-001` |
...
| **Output** | <code>_search_index.json</code> + <code>_search_index_emb.bin</code>, versioned by <code>pipeline/lib/search_index_metadata.py</code> |
| **Usage** | Natural-language query on topic page → the query embedding is computed for the reader by the worker <code>/api/embed</code> route (deployed) or <code>pipeline/serve_local.py</code> (local) with <code>gemini-embedding-001</code> (<code>task_type=RETRIEVAL_QUERY</code>) → **hybrid retrieval** (BM25 + dense, fused with RRF) → an LLM re-ranks the top candidates → user-key prefix auto-detected, and **Anthropic / OpenAI / Google** streams a grounded answer. Retrieval needs no reader key; a key (BYOK) is only for answer generation. Incompatible index/cache metadata is never mixed and never auto-rebuilt; run an explicit audit/migration/rebuild through the harness after reviewing the versioned metadata seam. Output is natural prose + clickable `[N]` citation chips + auto-inlined figures |
...
A fresh checkout includes built-in originality defaults; no external trigger JSON is required for the Originality section. Optional dependencies (Java, PaperBanana, local LLM endpoints) degrade to fallback or skipped optional features when absent. TLS verification is secure by default; prefer installing a trusted CA or setting `SSL_CERT_FILE`/`REQUESTS_CA_BUNDLE`. Temporary insecure TLS requires the named opt-out `PAPER_CURATION_INSECURE_TLS=1` or `network.allow_insecure_tls` with `network.insecure_tls_reason`.
...
# --max-papers limits only web search/register candidates; it is not a Zotero review or post-processing cap.
# This production command may auto-publish after success when Cloudflare credentials/config are present.
node ./bin/paper-curation.mjs run -- --topic <configured-topic> --mode curate --source web --days 7 --max-papers 20

# Local update — skip search, sync only, then review new papers (forced non-deploy)
PAPER_CURATION_NO_DEPLOY=1 node ./bin/paper-curation.mjs run -- --topic <configured-topic> --mode curate --source zotero --no-deploy

# Re-review specific slugs (audit/recovery), forced non-deploy
...
# Reclassify only (HDBSCAN approximate_predict + centroid fallback, no LLM calls; forced non-deploy)
PAPER_CURATION_NO_DEPLOY=1 node ./bin/paper-curation.mjs run -- --topic <configured-topic> --mode reclassify --no-deploy

# Also generate cross-category Research Insights (forced non-deploy)
PAPER_CURATION_NO_DEPLOY=1 node ./bin/paper-curation.mjs run -- --topic <configured-topic> --mode curate --source zotero --insights --no-deploy

# Regenerate timelines (narratives + images; forced non-deploy)
...
# Dry run — show execution plan with deploy suppression
PAPER_CURATION_NO_DEPLOY=1 node ./bin/paper-curation.mjs run -- --topic <configured-topic> --mode curate --source web --dry-run --no-deploy
...
`--mode` meanings:
- **curate** — review new papers only, preserve existing (most common); normal production runs may auto-publish after successful post-processing when Cloudflare credentials/config exist and deploy suppression is absent.
- **smoke** — scratch-only bounded Zotero/PDF/review smoke; forced no-deploy, no global post-processing.
...
Safety flags: `--strict-pdf` (block fuzzy PDF match), `--slugs A,B,C`, `--dry-run`, `--no-deploy`, `PAPER_CURATION_NO_DEPLOY=1`, `--skip-dedup`, `--dedup-execute`, `--insights`, `--yes`.
...
```bash
PAPER_CURATION_NO_DEPLOY=1 node ./bin/paper-curation.mjs run -- --topic <configured-topic> --mode curate --source web --since 2026-04-01 --until 2026-04-10 --skip-arxiv --no-deploy
```
...
PAPER_CURATION_NO_DEPLOY=1 node ./bin/paper-curation.mjs run -- --topic <configured-topic> --mode curate --source zotero --local-fallback --no-deploy
```
