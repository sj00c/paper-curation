...
A Google API key is optional: without it, figure validation and Audio Overview stay off and search runs BM25-lexical only (no dense retrieval). OpenAI is optional. Resend is needed only for deployed Audio Overview email. A missing optional key disables exactly that feature — nothing silently falls back to another vendor.
...
| **Processing (Option O-2, `--insights`)** | <ul><li>Cross-category Research Insights — one backend only. The candidate order (anthropic, openai, gemini) is a priority list, not a fallback chain: the first <em>configured</em> candidate runs and a failure ends there</li><li>Regenerates the network visualization (<code>network.html</code>)</li></ul> |
...
| `extract_insights` backend priority (no substitution) | Cross-category insights call exactly one backend: the first *configured* candidate from anthropic, openai, gemini. If it fails, the step fails — an unchosen vendor never answers in its place, because you cannot trust the provenance of a result you did not ask for and did not expect to be billed for. Skipping an unconfigured candidate is absence, which is allowed. With none configured the insights stay empty with `meta.status="unavailable"`. Order via `EXTRACT_INSIGHTS_CC_BACKENDS`; an unknown name fails loudly at import. |
...
| Category | Items |
|----------|-------|
| **Required** | Node.js 18+, Python 3.12 (`py312`), Zotero (API key + collection + PDFs) |
...
| **Optional** | Google API key (dense search embeddings, figure validation, TTS — absent means those stay off, not that another vendor takes over), OpenAI API key (reader BYOK answers, insights backend candidate), Resend API key (deployed Audio Overview email), Obsidian (notes/Graph View), PaperBanana (timeline images), Zotero Desktop (one-click PDF) |
