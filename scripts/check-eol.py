...
왜 필요한가. 이 저장소는 CRLF 파일과 LF 파일이 섞여 있다 (정확한 수는
`git ls-files --eol` 로 확인 — CRLF 17개, LF 다수) — upstream 원저자가 Windows 에서
만든 파일이 CRLF 로 커밋됐고, upstream 은 지금도 그렇게 커밋한다. 그 상태에서 편집
도구(또는 일괄 치환 스크립트)가 CRLF 파일을 LF 로 다시 저장하면, 두 줄 고친 변경이
파일 전체 재작성으로 잡힌다. 실제로 15개 파일 359줄 변경이 6,138줄로 부푼 적이 있다.
내용은 같은데 diff 가 쓰레기가 되어 리뷰가 불가능해진다.

왜 정규화하지 않는가. 전부 LF 로 바꾸면 9,912줄을 한 번에 재작성해야 하고, upstream
이 계속 CRLF 로 커밋하므로 머지할 때마다 같은 파일이 통째로 충돌한다. fork 를
유지하는 동안은 "있는 그대로 보존" 이 옳다.

.gitattributes 의 `* -text` 는 git 의 자동 변환만 막는다. 도구가 바이트를 바꿔버린
것은 못 막으므로 여기서 잡는다.
