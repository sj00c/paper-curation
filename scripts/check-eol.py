...
왜 필요한가. 이 저장소는 CRLF 17개와 LF 127개가 섞여 있다 — upstream 원저자가
Windows 에서 만든 파일이 CRLF 로 커밋됐고, upstream 은 지금도 그렇게 커밋한다.
그 상태에서 편집 도구(또는 일괄 치환 스크립트)가 CRLF 파일을 LF 로 다시 저장하면,
두 줄 고친 변경이 파일 전체 재작성으로 잡힌다. 실제로 15개 파일 359줄 변경이
6,138줄로 부푼 적이 있다. 내용은 같은데 diff 가 쓰레기가 되어 리뷰가 불가능해진다.
