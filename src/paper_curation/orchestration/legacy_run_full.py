...
PIPELINE = Path(__file__).resolve().parents[3] / "pipeline"
...
def main():
    ...
    # Execute the legacy tuple plan through the shared immutable graph.  Keep
    # `run()` as the executor seam used by existing callers and tests.
    ...
    steps = tuple(
        Step(
            ...
            side_effect=SideEffect.PUBLICATION if args.mode == "deploy" else SideEffect.LOCAL_WRITE,
            declared_effects=frozenset(
                {SideEffect.LOCAL_READ, SideEffect.NETWORK_READ}
                | ({SideEffect.PUBLICATION, SideEffect.LOCAL_WRITE}
                   if args.mode == "deploy" else {SideEffect.LOCAL_WRITE})
                | ({SideEffect.EXTERNAL_MUTATION}
                   if args.source == "web" or args.dedup_execute else set())
            ),
            failure_policy=FailurePolicy.CRITICAL if critical else FailurePolicy.OPTIONAL,
            timeout=timeout,
        )
        for index, (cmd, timeout, critical) in enumerate(plan, 1)
    )
    graph = Plan(
        "run_full",
        steps,
        local_only=not (
            args.mode == "deploy" or args.source == "web" or args.dedup_execute
        ),
    )

    def legacy_executor(argv, **kwargs):
        return run(argv, timeout=kwargs.get("timeout"), env=kwargs.get("env"))
    ...
    for i, step in enumerate(graph.topological_steps(), 1):
        try:
            receipt = runner.run(step)
        except StepExecutionError as exc:
            receipt = exc.receipt
            rc = receipt.returncode or 1
            if receipt.status.value == "timed-out":
                print(f"\n[run_full] step {i} timed out after {step.timeout}s (exit 124)")
