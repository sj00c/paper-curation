...
- **Whitelist .gitignore**: Everything is excluded by default (`*`), then only code + configs are whitelisted. Under `docs/` only explicitly whitelisted source documents/assets are tracked. Generated topic content under `docs/<configured-topic>/` and `docs/papers/` is gitignored and remains local until an explicit scope-checked deployment.
- **Generic topic presentation**: Topic labels come from `topic_profiles`; `review_to_html.py` and `build_topic_index.py` use the same generic theme/link behavior without built-in domain names.
