...
Usage:
  PYTHONUTF8=1 python pipeline/build_search_index.py --topic my-topic
  PYTHONUTF8=1 python pipeline/build_search_index.py --topic my-topic
  PYTHONUTF8=1 python pipeline/build_search_index.py --topic my-topic --limit 10    # debug
  PYTHONUTF8=1 python pipeline/build_search_index.py --topic my-topic --dry-run     # chunk only, no API
"""
...
# review.md 는 정량 디테일(수치/데이터셋/하이퍼파라미터)을 요약하며 떨군다.
# 원문 청크는 매몰 사실 질의의 적중률을 높인다. 단 원문 발췌라 저작권상 **배포 토픽엔 절대
...
def main():
    ...
    parser.add_argument("--topic", required=True, help="topic alias (e.g. my-topic)")
